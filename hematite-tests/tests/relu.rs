...
//
//! Per-op golden test: `KernelBackend::relu` through `RefBackend` (T5.1).
...
fn relu_golden() {
    ...
    let params = params_from_fixture!(relu_fixture);
    let mut output = [0i8; 8];
    backend
        .relu(&relu_fixture::INPUT_DATA, &params, &mut output)
